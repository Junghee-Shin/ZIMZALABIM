...
<body>
	<form name="frmMng" id="frmMng" action="/ZIMZALABIM/join/join.do" method="post" >
		...
		<div class="page-header">
			<center><h2>주최 상품 자세히보기</h2></center>
		</div>
		<!--// 제목 -->
		<h5>상품 상세보기로 이동하시려면 상품번호나 상품이름을 클릭해주세요.</h5>
		<br/>
        <input type="hidden"  name="work_div" id="work_div" />  
        <input type="hidden"  name="ProductNo" id="ProductNo" value="${vo.getProductNo() }" />
        <input type="hidden"  name="RegId" id="RegId" value="${vo.regId}"/>    
    	<label>상품번호</label>
    	<div><a href="javascript:move_Product();">${vo.getProductNo()}</a></div>
    	<br/>
		<label>상품이름</label>
    	<div><a href="javascript:move_Product();">${vo.getProductNm()}</a></div>
    	<br/>
		<label>주문수량</label>
    	<div><a>${vo.getTargetCnt()}</a></div>
    	<br/>
    	<label>주최일</label>
    	<div><a>${vo.getRegDt()}</a></div>
    	<br/>
    	<label>주최상태</label>
    	<div><a><%if(vo.getHostStatus().equals("20")){out.print("주최취소");}else if(vo.getHostStatus().equals("30")){out.print("주최완료");}else{out.print("주최진행");} %></a></div>
    	<br/>
		<label>배송상태</label>
		<div>
			<input type = "radio" name = "radioBtn" value = "100" <% if(vo.getDeliveryStatus().equals("100"))out.print("checked = 'checked'"); %> />결제 완료
			<input type = "radio" name = "radioBtn" value = "200" <% if(vo.getDeliveryStatus().equals("200"))out.print("checked = 'checked'"); %> />공구 진행
			<input type = "radio" name = "radioBtn" value = "300" <% if(vo.getDeliveryStatus().equals("300"))out.print("checked = 'checked'"); %> />배송 준비
			<input type = "radio" name = "radioBtn" value = "400" <% if(vo.getDeliveryStatus().equals("400"))out.print("checked = 'checked'"); %> />배송 완료
		</div>
	</form>
	<br/>
	<br/>
