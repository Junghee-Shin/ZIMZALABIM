...
<body>
	...
	<div class="container">
		...
		<form name="frm" id="frm"  method="post">
				...
				<input type="hidden" name="remaining" id="remaining" value="${vo.remaining}" />
			...
			<table class="table">
				<tr>
					...
					<td class="col-md-1"><fmt:formatNumber value="${vo.price}" pattern="#,###" /></td>
					<td class="text-left  lead col-md-1"><fmt:message key="D_REMAINING"></fmt:message></td>
					<td class="col-md-1" name="remaining" id="remaining">${vo.remaining}</td>
					<td class="text-left  lead  col-md-1"><fmt:message key="D_DEADLINE"></fmt:message></td>
					<td class="col-md-1" name="deadline" id="deadline" >${vo.deadline}</td>
				</tr>
			</table>
...
/*  		$(function(){
		...
		$(function(){ 
		  ...
		  $("#plus").click(function(){ 
			  //alert($("#remaining").text());
		   	 var join_cnt = document.frm.join_cnt;
		   	 //if(join_cnt<$("#remaining").text()){
			   	 join_cnt.value++; 
			 //}
...
// 						window.location.href="/ZIMZALABIM/mainpage/mainpage.do?work_div=do_move_to_main"; // 화면은 그대로 있고 데이터의 이동만 있음. 
		...
		$("#pay").click(function(){
			...
			//결제 수량 validation
  			if($("#join_cnt").val()>$("#remaining").val()){
  				//alert("$('#join_cnt').val():"+$("#join_cnt").val());
  				//alert("$(''#remaining').text():"+$("#remaining").val());
				alert("남은 수량을 확인하세요.");
				return ;
			}
			...
			//마감시간 validation
  			if($("#today").val()>$("#deadline").text()){
  				//alert("마감시간:"+$("#deadline").text());
  				//alert("현재날짜:"+$("#today").val());
				alert("마감시간이 지나 참여할 수 없습니다.");
				return ;
			}
			...
			$.ajax({
				...
				success: function(data){
					...
					if(null != jData && jData.msgId == "1"){
						alert(jData.msgContents);	
						window.location.href="http://localhost:8080/ZIMZALABIM/detail/detail.do?work_div=do_detail_select&productNo="+product_no; // 화면은 그대로 있고 데이터의 이동만 있음.
						//location.reload();
					}else{
						alert(jData.msgId+"|"+jData.msgContents);
					}
